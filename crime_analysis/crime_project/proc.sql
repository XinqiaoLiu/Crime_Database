...
create procedure GetDanger(in gender varchar(200), in age varchar(200),in Dname varchar(200),in st datetime, in et datetime, out r float)
    begin
      declare v_id int default 1;
      ...
      select sum(Level) into dangersum from (select Type from crime_app_crime where Area_id = v_id and Ctime>st and Ctime<et) A join (select Type,Level from crime_app_danger where Gender=gender and Age=age) B on A.Type = B.Type;
